...
def log(value, status='', color=''):
    value = colors('{color}[CC]WAIT {color}{status} [CC]{color}{value}[CC]'.format(
...
def log_replace(value, status='NPC TUBE', color=''):
    ...
    def start(self):
        try:
            ...
            if len(frontend_domains) == 0:
                ...
                return
            self.log('\n'.format(self.inject_host, self.inject_port))
            while True:
                socket_client, _ = socket_server.accept()
                socket_client.recv(4096)
                domain_fronting(socket_client, frontend_domains).start()
        except Exception as exception:
            self.log('GAGAL!!! Silahkan Mode Pesawat 2detik Tutup Qpython Dan Mulai Kembali'.format(self.inject_host, self.inject_port), color='[R1]')
...
G = '\033[1;96;3m'
print G + '   | SELAMAT DATANG DI NPC TUBE |               TELKOMSEL OPOK  2020             \n'
